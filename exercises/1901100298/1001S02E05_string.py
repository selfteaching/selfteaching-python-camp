...
print('将字符串里的 better 全部替换成 worse ==>', text)
...
filtered = []
# 用 for ... in 循环遍历一边 words 里的元素然后判断单词里面是否包含 ea
for word in words:
    #str类型的find方法 如果不包含参数 字符则返回-1，如果包含则返回该字符第一次出现时的索引
    if word.find('ea') < 0:
        filtered.append(word)
print('将单词中包含 ea 的单词剔除 ==>' , filtered) 

#3.进行大小写翻转
# 利用 列表推导式 对 str 类型的元素进行大小写翻转
swapcased = [i.swapcase() for i in filtered]
print('进行大小写翻转 ==>' , swapcased)

# 4.单词按 a...z 升序排列
print('单词按a...z顺序排列 ==>' , sorted(swapcased))
