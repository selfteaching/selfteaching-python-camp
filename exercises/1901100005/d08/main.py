...
from mymodule import stats_word         #从mydouble这个文件夹里调用stats_word文件里的stats_text程序
text = 123  #输入非字符串
try:                                    #try-except: 运行try代码判断try：如果try报错，则会执行except部分。
    print(stats_word.stats_text(text))  #如果try没有报错，程序会跳过except部分。
except ValueError as e :  #Except：捕获ValueError异常, ValueError输入一个调用者不期望的值， 'as e'将故障信息加载到变量e中。
    print(e) #e是错误的详细信息
