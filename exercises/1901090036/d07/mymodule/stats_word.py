...
#调用stats_text_en 和 stats_text_en ,输出合并词频统计结果
def stats_text(text): #定义函数
     counter=counter(stats_text_en(text))
     counter1=counter(stats_text_cn(text))
     merge_counter=counter(counter+counter1)#对counter和counter1进行合并
     merge_counter=sorted(merge_counter.items(),key=lambda x:x[1],reverse=True) #对新字典进行排序
