...
def stats_text_en(en_text):#处理英文文本，按单词出现次数，降序排列
    if not isinstance(en_text,str):#如果检测到en_text中的对象不是str字符串类型，与type()函数类似，推荐使用前者（type() 不会认为子类是一种父类类型，不考虑继承关系。isinstance() 会认为子类是一种父类类型，考虑继承关系。）
        raise ValueError("文本为非字符串类型")#抛出异常
