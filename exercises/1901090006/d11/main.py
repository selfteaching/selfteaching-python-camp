...
#读取文件
#with open(path+'/tang300.json') as f:
#    text = f.read()

#调用stats_word里的中文统计函数
#try:
#    print(stats_word.stats_text_cn(text,20))

#except ValueError as e:
#    print("oh,no,you're wrong",e.args)

response = requests.get('https://mp.weixin.qq.com/s/pLmuGoc4bZrMNl7MSoWgiA')
document = PyQuery(response.text)
cotents = document('#js_content').text()

sender = input('输入发件人邮箱:')
password = getpass.getpass('输入发件人邮箱密码(可复制粘贴):')

to = 'pythoncamp@163.com'
subject = '【慎艳玲】自学训练营DAY11 SYL-to-be-best'
body = str(stats_word.stats_text(cotents,100))
...
#print(stats_word.stats_text(text,20))