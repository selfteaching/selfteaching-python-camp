...
import sys
sys.path.append(r'C:\Users\hx\Documents\GitHub\selfteaching-python-camp\exercises\1901050037\d08\mymodule')
import stats_word
...
# 太行，王屋二山的北面，住了一個九十嵗的老翁，名叫愚公。二山佔地廣闊，擋住去路，使他和家人往來極爲不便。

# 一天愚公召集家人說：“讓我們各盡其力，鏟平二山，開條道路，直通豫州，你們認爲怎樣？”
# 大家都異口同聲贊成，只有他的妻子表示懷疑，并説：“你連開鑿一個小丘的力量都沒有，怎可能鏟平太行、王屋二山呢？況且，鑿出的土石又丟到哪裏去呢？”

# 大家都熱烈地說：“把土石丟進渤海裏。”
# 於是愚公就和兒孫，一起開挖土，把土石搬運到渤海裏。”
# 愚公的鄰居是個寡婦，有個兒子八歲也興致勃勃地走來幫忙。
# 寒來暑往，他們要一年才能往返渤海一次。

# 住在黃河河畔的智叟，看見他們這樣辛苦，取笑愚公說：“你不是很愚蠢嗎？你已一把年紀了，就是用盡你的氣力，也不能挖去山的一角？”

# 愚公嘆息道：“你有這樣的成見，是不會明白的。你比那寡婦的小兒子還不如呢！就算我死了，還有我的兒子，我的孫子，我的曾孫子，他們一直傳下去。而這二山是不會加大的，總有一天，我們會把他們鏟平。”
...
file_path = r"C:\Users\hx\Documents\GitHub\selfteaching-python-camp\exercises\1901050037\d09\mymodule\tang300.json"
word_lst1 = []
word_lst2 = []
word_lst3 = []
word_lst4 = []
with open(file_path, 'r', encoding='utf-8') as f:
    temp = json.load(f)
    for t in temp:
        contents_text=t["contents"].replace('，','').replace('。','').replace('！','').replace('？','').replace('\n','').replace('\t','').replace(' ','') #去掉特殊符号
        word_lst1.append(contents_text)
        text_cn1= "".join(word_lst1)
        
        type_text=t["type"].replace('，','').replace('。','').replace('！','').replace('？','').replace('\n','').replace('\t','').replace(' ','') #去掉特殊符号
        word_lst2.append(type_text)
        text_cn2= "".join(word_lst2)  
        
        author_text=t["author"].replace('，','').replace('。','').replace('！','').replace('？','').replace('\n','').replace('\t','').replace(' ','') #去掉特殊符号
        word_lst3.append(author_text)
        text_cn3= "".join(word_lst3)
        
        title_text=t["title"].replace('，','').replace('。','').replace('！','').replace('？','').replace('\n','').replace('\t','').replace(' ','') #去掉特殊符号
