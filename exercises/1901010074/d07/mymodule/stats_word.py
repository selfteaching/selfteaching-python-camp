...
⼀天，愚公召集家⼈說：「讓我們各盡其⼒，剷平⼆⼭，開條道路，直通豫州，你們認為怎 樣？」 ⼤家都異⼝同聲贊成，只有他的妻⼦表示懷疑，並說：「你連開鑿⼀個⼩丘的⼒量都沒有，怎 可能剷平太⾏、王屋⼆⼭呢？況且，鑿出的⼟⽯⼜丟到哪裏去呢？」

⼤家都熱烈地說：「把⼟⽯丟進渤海裏。」 於是愚公就和兒孫，⼀起開挖⼟，把⼟⽯搬運到渤海去。 愚公的鄰居是個寡婦，有個兒⼦⼋歲也興致勃勃地⾛來幫忙。 寒來暑往，他們要⼀年才能往返渤海⼀次。
...
智叟聽了，無話可說： ⼆⼭的守護神被愚公的堅毅精神嚇倒，便把此事奏知天帝。天帝佩服愚公的精神，就命兩位⼤ ⼒神揹⾛⼆⼭。
...
def stats_text_cn(text):
    ...
    cn=  sorted( result.items(),key= lambda item:item[1],reverse= True )
    print(cn)

def stats_text_en(en_text):
    result = {}
    en_text = re.sub("[^A-Za-z]","",en_text);
    for i in set(en_text):
        result[i] =en_text.count(i)
    #return sorted(result1.items(),key= lambda item:item[1],reverse = True)
    en= sorted(result.items(),key= lambda item:item[1],reverse = True)
    print(en)
