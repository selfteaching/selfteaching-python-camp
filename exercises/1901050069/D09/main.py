...
def load_file():
    '''
    1.
    下面是常用的获取文件路径的方式，要确保 tang300.json 跟当前文件在同一个文件夹下，这两种方式等价
    ...
    file_path = path.join(path.dirname(path.adspath(__file__)),'../tang300.json')
    print(__file__, file_path)
    3.
    这种方式表示 tang300.json 在当前文件的上一级目录的 data 文件夹下
    file_path = path.join(path.dirname(path.adspath(__file__)),'../data/tang300.json')
    print(__file__,file_path)
    '''
