...
# 验证参数检查功能是否生效
# result = stats_word.stats_text(text2)   
# print('统计结果==>', result)

# 捕获异常
try:
    stats_word.stats_text(text2)    # 首先尝试能运行吗
except ValueError:                  # 有ValueError之后的处理
    print('Error:文本非字符串')
...
if __name__ == "__main__":
    # stats_word.stats_text(1)  # 如果不注释掉，抛出的异常直接被程序打印出来了，test_traceback和test_logger没有被执行到。if有没捕获到异常，程序就退出了
    test_traceback()
    test_logger()