...
def stats_text_cn(text,count):
    ...
    counter_cn = {}
    character_cn_set = set(characters_cn)
    for character in character_cn_set:
        counter_cn[character] = characters_cn.count(character)
    #return sorted(counter_cn.items(),key = lambda x:x[1],reverse= True)
    return Counter(counter_cn).most_common(count)
