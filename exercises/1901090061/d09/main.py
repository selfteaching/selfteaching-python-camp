...
def load_file():
    '''
    1.
    下面是常用的获取文件路径的方式，要确保 tang300.json 跟当前文件在同一个文件下下面，这两种方式等价
    file_path = path.join(path.dirname(path.abspath(__file__)),'./tang300.json')
    '''
    file_path = path.join(path.dirname(path.abspath(__file__)),'tang300.json')
    print('当前文件路径:',__file__,'\n读取文件路径:',file_path)
    '''
    2.这种方式表示 tang300.json 在当前文件的上一级目录
    file_path = path.join(pathn.dirname(path.abspath(__file__)),'../tang300.json')
    print(__file__,filepath)
    
    3.这种表示方式 tang300.json 在当前文件的上一级目录下的data文件夹下
