...
def isStr(text):
    if not isinstance(text,str):
        raise ValueError("text parameter is not string ,please input text of string")

#定义一个名字为stats_text_en的函数，功能为统计出现单词的个数
def stats_text_en(strText,count):
    isStr(strText)
    #创建一个空字典用来统计单词个数
    #dic={}
    #将文本中的非单词因素去掉
    ...
    #使用collections.Counter来排序
    list3=Counter(list2).most_common(count)
    return list3

#封装统计中文汉字字频的函数
def stats_text_cn(textC,count=None):
    isStr(textC)
    list2=[]
    seg_list=jieba.cut(textC)
    text="/".join(seg_list)
    text=text.split("/")
    for ss in text:
        if len(ss)>=2:
...
def stats_text(text,count):
    ...
    textC=stats_text_cn(text,count)
    print(textC)
