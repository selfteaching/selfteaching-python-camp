...
def stats_text_cn(text,count):        #定义统计中文字符的函数
    if type(text )!=str:
        raise ValueError('输入的参数为非字符串')
    text=re.findall(r'[\u4e00-\u9FA5]',text)    #利用正则表达式按unicode编码将中文提取出来  
    text=''.join(text)   
    text= jieba.lcut(text)
    
    text_list=[]              #建立空列表
    
    for i in text:            #用for 遍历分词
        if len(i)>=2:       #只取长度大于2的词汇
            text_list.append(i)          #将其添加到列表里          
    counts=Counter(text_list).most_common(count) 
