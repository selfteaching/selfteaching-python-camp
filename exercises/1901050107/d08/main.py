from mymodule import stats_word  #模块导入mymodule目录下的stats_word.py文件
...
大家都異口同聲贊成，只有他的妻子表示懷疑，並說：「你連開鑿一個小丘的力量都沒有，怎 可能剷平太行、王屋二山昵？況且，鑿出的土石又丟到哪裏去昵？」
...
愚公歎息道：「你有這樣的成見，是不會明白的。你比那寡婦的小兒子還不如昵！就算我死 了，還有我的兒子，我的孫子，我的曾孫子，他們一直傳下去。而這二山是不會加大的，總有 -天，我們會把它們剷平。」
...
# 调⽤用 stats_word 中的任何⼀一个函数，参数传⼊入⾮非字符串串，验证参数检查
# 功能是否⽣生效
