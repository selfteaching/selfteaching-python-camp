...
和家人往來極為不便。
一天，愚公召集家人說：「讓我們各盡其其力，剷平二山，開條道路，直通豫州，你們認為怎
樣？」
大家都異口同聲贊成，只有他的妻子表示懷疑，並說：「你連開鑿一個小丘的力量都沒有，怎
可能剷平太行和王屋二山呢？況且，鑿出的土石又丟到哪裏去呢？」
⼤家都熱烈地說：「把土石丟進渤海裏。」
於是愚公就和兒孫，一起開挖土，把土石搬運到渤海去。
愚公的鄰居是個寡婦，有個兒子八歲也興致勃勃地走來幫忙。
寒來暑往，他們要一年才能往返渤海一次。
住在黄河河畔的智叟，看见他們這樣辛苦，取笑愚公說：「你不是很愚蠢嗎？你已一把年紀
...
'''
参数sample_text用调用的函数stats_text的方法处理，
然后成为导入的文件名stats_text的list，把每个返回值，赋值给result
视情况，可能需要对原始模的被调用函数做一定修改调整，
比如把stats_text_cn和stats_text_en,合并成stats_text
'''
print('统计结果 ==>',result) 