...
list_text = [1,2,3,4,5] #新设定的列表

try:
    list_text is isinstance(list_text,str) #如果取值内容为字符串
    print(stats_word.stats_text(list_text)) #通过模块内的函数stats_text()罗列字符
except ValueError: #如果取值不正确，不是字符串
    print('输入文本为非字符串，请再次检查第67行代码的取值') #则打印报错提示