...
# set (集合) 类型 可以去掉 列表 里重复元素，可以在 for...in 里减少循环次数
word_set = set(words)
...
# 内置函数 sorted 的参数 key 表示按元素的那一项的值进行排列
# dict 类型 counter 的 items 方法会返回一个 包含 相应 项 (key, value) 的 元素 列表
