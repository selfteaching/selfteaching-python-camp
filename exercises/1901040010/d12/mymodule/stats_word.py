...
太⾏，王屋⼆山的北面，住了了一個九十歲的⽼翁，名叫愚公。⼆山佔地廣闊，擋住去路，使他和家⼈人往來極為不便。

一天，愚公召集家人說:「讓我們各盡其力，剷平⼆山，開條道路，直通豫州，你們認為怎樣?」
⼤家都異口同聲贊成，只有他的妻⼦子表示懷疑，並說:「你連開鑿一個小丘的⼒量都沒有，怎可能剷平太行、王屋⼆山呢?況且，鑿出的⼟石又丟到哪裏去呢?」

大家都熱烈地說:「把⼟石丟進渤海裏。」
於是愚公就和兒孫，一起開挖土，把⼟石搬運到渤海去。
愚公的鄰居是個寡婦，有個兒⼦八歲也興致勃勃地⾛來幫忙。
寒來暑往，他們要一年才能往返渤海一次。

住在⿈河畔的智叟，看⾒他們這樣辛苦，取笑愚公說:「你不是很愚蠢嗎?你已一把年紀了，就是⽤盡你的氣⼒，也不能挖去山的⼀角呢?」

愚公歎息道:「你有這樣的成⾒，是不會明白的。你⽐那寡婦的⼩兒⼦還不如呢!就算我死了，還有我的兒子，我的孫子，我的曾孫子，他們一直傳下去。而這⼆山是不會加大的，總有
⼀天，我們會把它們剷平。」
智叟聽了，無話可說:
⼆山的守護神被愚公的堅毅精神嚇倒，便把此事奏知天帝。天帝佩服愚公的精神，就命兩位大⼒神揹⾛⼆山。
...
def stats_text_cn(text,count):
    ...
    text = [x for x in jieba.cut_for_search(text) if len(x) >= 2]
    words_cn = []
    count = int(count)
    for word in text:
        if '\u4e00' <= word <= '\u9fff':         #检查是否为中文字符
            words_cn.append(word)   
    result = Counter(words_cn).most_common(count)
    return result
