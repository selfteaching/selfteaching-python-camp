text = '''
愚公移⼭
太行，王屋⼆山的北面，住了一個九十歲的⽼翁，名叫愚公。⼆山佔地廣闊，擋住去路路，使他和家⼈往來極為不便。
⼀天，愚公召集家⼈人說：「讓我們各盡其力，剷平⼆山，開條道路，直通豫州，你們認為怎樣？」
⼤家都異異⼝同聲贊成，只有他的妻子表示懷疑，並說：「你連開鑿一個⼩丘的⼒量都沒有，怎可能剷平太行、王屋⼆山呢？況且，鑿出的⼟石⼜丟到哪裏去呢？」
⼤家都熱烈地說：「把⼟石丟進渤海海裏。」
於是愚公就和兒孫，⼀起開挖土，把⼟石搬運到渤海去。愚公的鄰居是個寡婦，有個兒⼦八歲也興致勃地⾛來幫忙。
寒來暑往，他們要一年才能往返渤海一次。
住在⿈河畔的智叟，看見他們這樣辛苦，取笑愚公說：「你不是很愚蠢嗎？你已一把年紀了，就是⽤盡你的氣力，也不能挖去山的一角呢？」
愚公歎息道：「你有這樣的成見，是不會明白的。你⽐那寡婦的⼩兒子還不如呢！就算我死了，還有我的兒子，我的孫⼦，我的曾孫⼦，他們一直傳下去。⽽這⼆山是不會加大的，總有
⼀天，我們會把它們剷平。」
智叟聽了，無話可說：
⼆山的守護神被愚公的堅毅精神嚇倒，便把此事奏知天帝。天帝佩服愚公的精神，就命兩位⼤力神揹走⼆山。
...
import re
import collections
count=int()  #int() 函数用于将一个字符串或数字转换为整型
def stats_text_cn(text,count):
    ...
    #Counter为每个出现的英文单词计数，返回前count个词频最高的词及出现的次数.
    #以int整数类型作为输出限制，使输出的元素count为整数。
