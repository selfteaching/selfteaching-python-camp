...
def load_file():
    '''
    1.
    下面是常用的获取文件路径的方式，要确保tang300.json跟当前文件在同一个文件夹下，这两种方式等价
    file_path=path.join(path.dirname(path.abspath(_file_)),'./tang300.json')
    '''
    ...
    file_path=path.join(path.dirname(path.abspath(__file)),'../tang300.json')
