...
for element in elements:
    ...
    for symbol in symbols:
        # 逐个替换字符串，用""是为了同时删除符号所占的位置
        element = element.replace(symbol," ")
    # 删除字符后如果 element 的长度不为 0 ，则算作正常单词
    if len(element):
...
# set(集合) 类型可以去掉 列表 里的重复元素，可以在 for ... in 里减少循环次数
word_set = set(words)

for word in word_set:
    counter[word] = words.count(word)
print("英文单词的出现次数 ==>", counter)
...
print("从⼤到⼩输出所有的单词及出现的次数 ==>", sorted(counter.items(),key = lambda x: x[1], reverse = True))
