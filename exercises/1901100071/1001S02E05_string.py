...
print("将字符串样本 text ⾥的 better 全部替换成 worse ==>", text_1)

# 2.将单词中包含 ea 的单词剔除

# 先将字符串根据 空白字符 分割成 list, 要调用 str 类型
words = text_1.split()
# 定义一个 list 类型变量用来存放过滤完的单词
filtered = []
# 用 for...in循环遍历一遍 text_1 的元素然后判断单词是否包含 es
for word in words:
...
# print("所有单词按 a…z 降序排列 ==>",sorted(swapcased), reverse = True)
