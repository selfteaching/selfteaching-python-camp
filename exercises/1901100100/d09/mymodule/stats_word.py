...
def stats_text_en(text,num):
    ...
    for j in list_en:
        ...
        if word != "":
            words.append(word)
    words_order = Counter(words).most_common(num)
    return words_order
...
def stats_text_cn(text,num):
    ...
    for i in text:
        if u'\u4e00' <= i<=u'\u9fff':
            words_ch.append(i)
    words_order = Counter(words_ch).most_common(num)
    return words_order
...
def stats_text(text,n):
    ...
    result_en = stats_text_en(text,n)
    result_ch = stats_text_cn(text,n)
    result_en += result_ch
    
    return result_en
    
if __name__ == '__main__':
    result_all = stats_text(text,15)
    print(result_all)
