...
def stats_text_cn():
    ...
    import collections
    import json
    with open('/Users/mayjiao/Documents/GitHub/selfteaching-python-camp/exercises/1901100169/D10/mymodule/tang300.json','r',encoding='utf-8') as f:
        k=f.read()
        x="""*-,.!@#$%^&*()_+|}{:"~`\/][; 。，！？［］
        ...
        for c in x: #按每个标点在X里面循环
            k=k.replace(c,'')
        i=jieba.cut (k) 
        q=collections.Counter(i).most_common(20)
        print(q)
    a=jieba.cut_for_search(text1)
    c=[]
    for b in a:
        if len(b)>=2:
            c.append(b)         
