...
def stats_text_cn(url,count):
    ...
    content1 = [x for x in jieba.cut(content) if len(x) >= 2]  #使用精确模式分词
    q=collections.Counter(content1).most_common(count)
    a=str(q)
#    print(a)
#    sender = 'lostsoulcry@gmail.com'
    # password = 'Dvnuars1238^_^'
    # recipients = '75715090@qq.com'
...
def send_mail():
    ...
    content=stats_text_cn(url,100)
    yag=yagmail.SMTP(user=sender,password=password,host='smtp.163.com')
    yag.send(to=recipients,subject="1901100169 自学训练营17群 DAY11 MayJiao",contents=content)
    return
send_mail()
