...
# 调用 ''(空字符串) str 类型的 join 方法可以连接列表里的元素，用''(空字符串)表示连接的时候元素健不用任何字符隔开
# 因为 reversed_list 里面都是 int 类型元素， 所以拼接之前要把 reversed_list 变成一个包含 str 类型元素的列表
joined_str = ''.join([str(i) for i in reversed_list])
print('翻转后的数组拼接成字符串 ==>', joined_str)

# 3. 用字符串切片的方式去除第三道第八个字符（包含第三和第八个字符）

# 切片的时候包含 开始 索引， 不包含结尾的索引
sliced_str = joined_str[2:8]
print('用字符串切片的方式去除第三道第八个字符 ==>', sliced_str)
