...
def stats_text_en(text): # 定义函数
    ...
    if type(text)!=str:                   #添加参数型检查，检查是否为非字符串。
    ...
    for i in a:
        ...
        r1 = {i:count} 
        b.update(r1)
    c = sorted(b.items(),key=lambda x:x[1],reverse=True) #降序排序
...
def stats_text_cn(text):
    ...
    for j in d:
        ...
        r2 = {j:count}
        e.update(r2)
    f = sorted(e.items(),key=lambda x:x[1],reverse=True)#降序排序
    print('中文单字统计频率如下： \n',f)
