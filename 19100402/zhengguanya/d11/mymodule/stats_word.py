...
樣？」
大家都異口同聲贊成，只有他的妻子表示懷疑，並說：「你連開鑿一個小丘的力量都沒有，怎
可能剷平太行、王屋二山呢？況且，鑿出的土石又丟到哪裏去呢？」
大家都熱烈地說：「把土石丟進渤海裏。」
於是愚公就和兒孫，一起開挖土，把土石搬運到渤海去。
愚公的鄰居是個寡婦，有個兒子八歲也興致勃勃地走來幫忙。
寒來暑往，他們要一年才能往返渤海一次。
住在黃河河畔的智叟，看見他們這樣辛苦，取笑愚公說：「你不是很愚蠢嗎？你已一把年紀
了，就是用盡你的氣力，也不能挖去山的一角呢？」
愚公歎息道：「你有這樣的成見，是不會明白的。你比那寡婦的小兒子還不如呢！就算我死
...
def stats_text(i):
       if type(i)==str:
              stats_text_cn(i)
              stats_text_en(i)
              print(stats_text_cn(i),stats_text_en(i))
              return
       else:
              raise ValueError(i)      
...
def stats_text_en(x):
       if type(x)==str:
              result = re.sub("[^a-zA-Z]"," ",x.strip())
              newList = result.split()
              return collections.Counter(newList),'\n'
       else:
              raise ValueError(x)
...
def stats_text_cn(y):
       if type(y)==str: 
              result1 = re.findall(u'[\u4e00-\u9fff]+',y)
              newList1 = ''.join(result1)
              
              seg_list = jieba.cut(newList1,cut_all=False)
              dic = [n for n in seg_list]
              dic1 = []
              for n in dic:
                     if len(n)>=2:
                            dic1.append(n)

              return collections.Counter(dic1).most_common(100),'\n'
