...
'''用正则去掉文本中的数字，字母和符号'''
content = re.sub(r'[a-zA-Z0-9’!"%&\'^\s+|\s+$()*+,-./:;，。?、…？“”‘’！]+',"",content)
'''用jieba将文本进行全模式切分'''
...
password=getpass.getpass('请输入发件人邮箱密码(可复制粘贴):'),host='smtp.126.com')  
sendSmpt.send(to = input('请输入收件人邮箱:'),subject="19100103 zdy958",contents=mystr)