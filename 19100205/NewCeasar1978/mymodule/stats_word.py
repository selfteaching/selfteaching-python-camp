...
def stats_text(text):
    ...
    for i in text:
        if '\u4e00' <= i <= '\u9fff':
            text_cn += i
        elif i not in symbol:
            text_en += i
    d_cn = stats_text_cn(text_cn)
    d_en = stats_text_en(text_en)
    d = d_cn + d_en
    return d

def stats_text_1(text):
    """统计输入文本中的中英文词频"""
    ...
    list_total = list_cn + list_en
    from collections import Counter
    result = Counter(list_total).most_common(n)
    return result

def stats_text_2(text):
    """统计输入文本中的中英文词频"""  
    try:
        if type(text) != str:
            raise ValueError
        ...
        print('输入类型错误')
    n = int(input('输入要列出的结果数量：'))
    m = int(input('输入要列出的最低词语长度：'))
    import jieba
    list_a = []
    symbol = ['，','。','？','！','：',';','「','」','：'\
            ',','.','?','!',':','\"','--','*','(',')','“','”','、','{','}','\\n\\n\\n',' ','\n','\\','n',' ','\n']
    for i in jieba.cut(text):
        if i not in symbol and len(i) >= m:
            list_a.append(i)
    from collections import Counter
    return Counter(list_a).most_common(n)
    for i in Counter(list_a).most_common(n):
        print(i[0],':',i[1],'次')
def stats_text_3(text):
    """统计输入文本中的中英文词频"""  
    try:
        if type(text) != str:
            raise ValueError
    ...
    symbol = ['，','。','？','！','：',';','「','」','：'\
            ',','.','?','!',':','\"','--','*','(',')','“','”','、','{','}','\\n\\n\\n',' ','\n','\\','n',' ','\n']
    for i in jieba.cut(text):
        if i not in symbol and len(i) >= 2:
            list_a.append(i)
    from collections import Counter
    return Counter(list_a).most_common(20)
    for i in Counter(list_a).most_common(10):
        print(i[0],':',i[1],'次')
